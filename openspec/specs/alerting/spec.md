...
## Purpose
TBD - created by archiving change add-notifiers. Update Purpose after archive.
## Requirements
...
In **`single`** auth mode this scoping is vacuous and SHALL NOT be described to the operator as
though it were protection: there is no login wall, `current_user` resolves the one implicit user
unconditionally, and therefore *anyone who can reach the panel's address* is the owner and can open
any review link. Access in that mode is controlled entirely by whatever fronts the panel (a reverse
proxy or SSO). The routing UI SHALL say so plainly, because an operator who forwards an alert email
is trusting that sentence — telling them the link is owner-scoped would invite them to disclose
their files to someone Cairn will not turn away.

In **`multi`** mode the scoping is real: a recipient who is not the owner reaches a "not found"
page. The routing UI SHALL make that expectation visible rather than let the link silently
disappoint a recipient.

That `multi`-mode gap is an accepted limitation, recorded so it is a known property rather than a
surprise: solving it properly means per-collection sharing or recipient-to-user mapping, which is
Phase-2 auth work.
...
- **WHEN** in `multi` auth mode, an authenticated user who does not own the collection opens the
  review link
- **THEN** the response SHALL remain the existing owner-scoped "not found", disclosing nothing about
  whether the collection exists

#### Scenario: Single mode does not claim protection it does not provide

- **WHEN** an operator configures alert recipients while the system is in `single` auth mode
- **THEN** the panel SHALL state that Cairn does not gate the review link itself and that whatever
  fronts the panel controls access
- **AND** it SHALL NOT describe the link as owner-scoped

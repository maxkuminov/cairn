...
- [x] 1.1 New `src/services/panel_url.py` (or an equivalent single module) with
  ...
  non-empty value.
- [x] 1.2 `panel_link(public_url, path) -> str | None` — `None` when `public_url` is falsy, else the
  base joined to `path` with exactly one slash. The **only** place links are built.
- [x] 1.3 `src/config.py`: add `public_url: str | None = None`. Its validator is **fail-soft** —
...
- [x] 2.1 `src/services/app_settings.py`: overlay `public_url` alongside the SMTP fields (keep
  `SMTP_FIELDS` meaning what it says — add e.g. `OVERLAY_FIELDS = SMTP_FIELDS + ("public_url",)`).
- [x] 2.2 **Validate on read.** `model_copy(update=...)` does not re-run validators, so
...
- [x] 2.3 `save_public_url(session, url)`: normalize (raising `ValueError` for the route to catch)
...
## 3. Alert payload
- [x] 3.1 `src/notify/base.py`: add `url: str | None = None` to `Alert` (default keeps both existing
  construction sites valid).

## 4. Channels — link-free output must stay byte-identical
- [x] 4.1 `src/notify/smtp.py`: when `alert.url` is set, plaintext gains a
...
- [x] 4.2 `src/notify/webhook.py`: include `"url"` **only when set** — omit the key entirely
  otherwise (never `"url": null`; a strict consumer may reject it, and a rejected webhook is a
  silently missed alert).
- [x] 4.3 `src/notify/ntfy.py`: set the `Click` header only when a link exists, and only if the URL
  is header-safe (no control characters — the grammar already excludes them; assert, don't trust);
  append the link to the body.
- [x] 4.4 `src/notify/signal_callmebot.py`: append the link to the message text when present.
- [x] 4.5 `src/notify/kuma_push.py`: its `msg` query parameter already carries human-readable text
  (`"{summary} in {collection_name}"`), so append the link there when present. (The earlier "no
  message body" premise was wrong.)

## 5. Dispatch sites
- [x] 5.1 `src/services/scanner.py` (~line 548): build the link in **its own** `try/except` that
...
- [x] 5.2 `src/control_panel/routes.py` `settings_smtp_test`: pass a link to `/collections` so the
  test email exercises the configured address before a real incident depends on it.

## 6. Panel
- [x] 6.1 `settings.html`: admin-only "Panel address" field in its own card above the email channel
...
- [x] 6.2 `routes.py`: `settings_page` passes the effective `public_url` into the context and derives
...
## 7. Docs
- [x] 7.1 Document `CAIRN_PUBLIC_URL` in `config.example.yaml`, `docker-compose.example.yml`, and
  `DEPLOYMENT.md` — it must be the address a *human* reaches (the reverse-proxy URL), and it is
  optional.
- [x] 7.2 Add a line to the alerting note in `CLAUDE.md`, including the owner-scoping limitation.
- [x] 7.3 Collection alert-routing UI: a hint that the review link is actionable only by the
  collection's owner (relevant in `multi` mode; harmless in `single`).

## 8. Tests
### Validation & config
- [x] 8.1 `normalize_public_url`: accepts http/https with host, port, and path prefix; strips
...
- [x] 8.2 A malformed `CAIRN_PUBLIC_URL` **does not raise** — `Settings` builds with `public_url is
  None` (the startup-safety guarantee).
- [x] 8.3 `panel_link` joins with exactly one slash and preserves a path prefix.
### Overlay
- [x] 8.4 `effective_settings` overlays a stored `public_url` over env (DB wins); falls back when the
...
- [x] 8.5 SMTP with a link: plaintext contains the absolute URL; the message is
  `multipart/alternative`; the HTML part's anchor `href` matches.
- [x] 8.6 SMTP without a link: message is a **single `text/plain` part** (not multipart) and matches
  the previous wording.
- [x] 8.7 SMTP escaping: a path containing `<img src=x onerror=alert(1)>.txt` and a collection name
  containing `&`/`<` render as literal text in the HTML part; the plaintext part is unaffected.
- [x] 8.8 Webhook: payload contains `url` when linked, and the key is **absent** (not null) when not.
- [x] 8.9 ntfy: `Click` header set when linked, header **absent** when not.
- [x] 8.10 Signal: link appended when present, text unchanged when absent.
- [x] 8.10a Kuma: `msg` param includes the link when present, unchanged when absent.
- [x] 8.10b A non-ASCII `public_url` is either ASCII-normalized or rejected, and every channel that
...
### Dispatch
- [x] 8.11 A scan that triggers an alert dispatches an `Alert` whose `url` is
  `{public_url}/collection/{id}/review` (stubbed dispatch).
- [x] 8.12 **Failure injection:** make the link builder raise; assert `dispatch` is still called
  once, with `alert.url is None`, and the run still records its result.
### Panel
- [x] 8.13 `POST /settings/panel-url` rejects an invalid URL without changing the stored value and

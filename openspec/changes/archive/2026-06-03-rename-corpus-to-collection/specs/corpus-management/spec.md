# corpus-management Specification (delta)
...
(`verify_cadence_seconds`, where `0` disables deep verify), and exclude globs. For backward
compatibility the `cairn add-collection` command SHALL accept `add-corpus` as an alias, and the
`--collection` option SHALL accept `--corpus` as an alias.
...
- **THEN** a `collections` row SHALL be created owned by the implicit single user, storing the
  resolved absolute root and the chosen mode/ots-mode/cadence/verify-cadence/excludes
